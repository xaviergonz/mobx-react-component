...
test("uncommitted components should not leak observations", async () => {
    ...
    const TestComponent1 = mobxObserver(() => <div>{store.count1}</div>)
    const TestComponent2 = mobxObserver(() => <div>{store.count2}</div>)
...
test("cleanup timer should not clean up recently-pended reactions", () => {
    ...
    const TestComponent1 = mobxObserver(() => <div>{store.count}</div>)
